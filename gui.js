(async () => {
    ...
    if (console.log("%c Glaringly's Blooket Hacks %c\n\tModified by Glaringly", "color: #cc0022; font-size: 3rem; text-shadow: 0 0 20px #cc0022, 0 0 40px rgba(204,0,34,0.5)", "color: #ff4422; font-size: 1rem"), console.log("%c\tgui.js", "color: #cc0022; font-size: 1rem"), console.log("%c\tStar the github repo!%c  https://github.com/LieutenantGst/Ghsts-Blooket-Cheat", "color: #ff6666; font-size: 1rem", ""), document.querySelector("script[src*='bfs/index.js']") && !window.clearId) {
    ...
    (() => {
        ...
        link.setAttribute('href', 'https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Rajdhani:wght@400;600;700&family=Share+Tech+Mono&display=swap');
    ...
    let c = {
        ...
        setItem(e, t) {
            return e.split(".").reduce((e, a, o, r) => (e[a] = (e[a] ? e[a] : {}), ++o == r.length && (e[a] = t), e[a]), this.data),
                localStorage.setItem("GLARINGLYSettings", JSON.stringify(this.data)),
                this.data
        },
        deleteItem(e) {
    ...
    try {
        for (let d of (c.data = JSON.parse(localStorage.getItem("GLARINGLYSettings") || "{}"), ["backgroundColor", "cheatList", "contentBackground", "defaultButton", "disabledButton", "enabledButton", "infoColor", "inputColor", "textColor"]))
    ...
    let p,
        ...
        _ = l("div", {
            ...
            id: "variables",
            innerHTML: `:root {--backgroundColor: ${c.data?.theme?.backgroundColor || "#050a1a"};--infoColor: ${c.data?.theme?.infoColor || "#0d1f3c"};--cheatList: ${c.data?.theme?.cheatList || "#080f24"};--defaultButton: ${c.data?.theme?.defaultButton || "#1a1050"};--disabledButton: ${c.data?.theme?.disabledButton || "#5c1526"};--enabledButton: ${c.data?.theme?.enabledButton || "#0d4a2e"};--textColor: ${c.data?.theme?.textColor || "#e0f7ff"};--inputColor: ${c.data?.theme?.inputColor || "#0a1535"};--contentBackground: ${c.data?.theme?.contentBackground || "#070d20"};--neonCyan: #cc0022;--neonPink: #ff2d78;--neonPurple: #bf00ff;}`
        }), l("style", {
            innerHTML: `@import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700;900&family=Rajdhani:wght@400;600;700&family=Share+Tech+Mono&display=swap');
*{box-sizing:border-box}
.alertList::-webkit-scrollbar{display:none}.alertList{-ms-overflow-style:none;scrollbar-width:none}
.contentWrapper::-webkit-scrollbar{width:3px}.contentWrapper::-webkit-scrollbar-track{background:transparent}.contentWrapper::-webkit-scrollbar-thumb{background:var(--crimson);border-radius:3px;box-shadow:0 0 6px var(--crimsonGlow)}.contentWrapper{scrollbar-width:thin;scrollbar-color:var(--crimson) transparent}
.cheatButton{position:relative;display:flex;flex-direction:row;align-items:center;min-height:44px;width:200px;margin:1px 0;padding-left:26px;box-sizing:border-box;cursor:pointer;user-select:none;text-decoration:none;border-radius:0;background-color:transparent;color:rgba(255,200,200,0.55);transition:all .18s ease;font-size:12px;font-weight:700;font-family:'Rajdhani',sans-serif;letter-spacing:0.1em;text-transform:uppercase;border-right:2px solid transparent}
.cheatButton::before{content:'';position:absolute;left:0;top:50%;transform:translateY(-50%);width:2px;height:0;background:var(--crimson);box-shadow:0 0 10px var(--crimsonGlow),0 0 20px var(--crimsonGlow);transition:height .2s cubic-bezier(.4,0,.2,1);border-radius:0 1px 1px 0}
.cheatButton::after{content:'';position:absolute;right:0;top:0;width:0;height:100%;background:linear-gradient(90deg,transparent,var(--crimsonDim));transition:width .2s ease;pointer-events:none}
.cheatButton:hover{color:#ff6666;border-right-color:rgba(204,0,34,0.4);text-shadow:0 0 12px rgba(204,0,34,0.9)}
.cheatButton:hover::before{height:80%}
.cheatButton:hover::after{width:100%}
.cheatButton.glare-active{color:var(--crimson);border-right-color:var(--crimson);text-shadow:0 0 15px var(--crimsonGlow);background:linear-gradient(90deg,var(--crimsonDim),transparent)}
.cheatButton.glare-active::before{height:80%}
.cheatInput,select{min-width:185px;padding:7px 12px;font-family:'Share Tech Mono',monospace;font-weight:400;font-size:12px;background-color:rgba(204,0,34,0.04);border:1px solid rgba(204,0,34,0.2);margin:4px;color:#ff9999;transition:all .2s;caret-color:var(--crimson);letter-spacing:0.04em;border-radius:2px}
.cheatInput:focus,select:focus{outline:0;border-color:rgba(204,0,34,0.7);box-shadow:0 0 15px rgba(204,0,34,0.2),inset 0 0 8px rgba(0,0,0,0.4)}
.cheatInput::placeholder{color:rgba(204,0,34,0.3)}
.cheatInput::-webkit-inner-spin-button,.cheatInput::-webkit-outer-spin-button{-webkit-appearance:none;margin:0}
.cheatInput[type=number]{-moz-appearance:textfield}
.bigButton:hover{filter:brightness(120%);transform:translateY(-2px)}
.bigButton:active{transform:translateY(1px)}
.cheatList::-webkit-scrollbar{width:2px}.cheatList::-webkit-scrollbar-track{background:transparent}.cheatList::-webkit-scrollbar-thumb{background:var(--crimson);border-radius:2px;box-shadow:0 0 8px var(--crimsonGlow)}
.scriptButton{align-items:center;box-sizing:border-box;display:flex;flex-direction:column;justify-content:center;margin:7px;padding:13px 10px;position:relative;width:230px;font-family:'Rajdhani',sans-serif;font-weight:700;color:#ffcccc;border-radius:2px;cursor:pointer;transition:all .25s cubic-bezier(.4,0,.2,1);border:1px solid rgba(204,0,34,0.15);background:var(--defaultButton);overflow:hidden;letter-spacing:0.08em;text-transform:uppercase}
.scriptButton::before{content:'';position:absolute;top:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,var(--crimson),transparent);transform:scaleX(0);transition:transform .3s ease;transform-origin:center}
.scriptButton::after{content:'';position:absolute;inset:0;background:linear-gradient(135deg,transparent 40%,rgba(204,0,34,0.04));opacity:0;transition:opacity .25s}
.scriptButton:hover{border-color:rgba(204,0,34,0.5);box-shadow:0 0 25px rgba(204,0,34,0.1),inset 0 0 25px rgba(0,0,0,0.3);color:#ff8888}
.scriptButton:hover::before{transform:scaleX(1)}
.scriptButton:hover::after{opacity:1}
.scriptButton:active{transform:scale(0.98)}
.tooltip::after{content:"";position:absolute;width:6px;height:6px;background-color:inherit;top:-3px;left:50%;margin-left:-3px;transform:rotate(45deg);border-top:1px solid rgba(204,0,34,0.3);border-left:1px solid rgba(204,0,34,0.3)}
.cheatName{font-size:14px;font-weight:700;text-align:center;letter-spacing:0.12em;text-shadow:0 0 15px rgba(204,0,34,0.6);color:#ffaaaa}
select{text-align:center;cursor:pointer}
select option{background:#080000;color:#ff9999}
@keyframes glare-pulse{0%,100%{box-shadow:0 0 5px var(--crimsonGlow)}50%{box-shadow:0 0 20px var(--crimsonGlow),0 0 40px rgba(204,0,34,0.2)}}
@keyframes glare-flicker{0%,100%{opacity:1}92%{opacity:1}93%{opacity:0.6}94%{opacity:1}96%{opacity:0.8}97%{opacity:1}}
@keyframes glare-scan{0%{transform:translateY(-100%)}100%{transform:translateY(100vh)}}
@keyframes glare-slideIn{from{opacity:0;transform:translateX(-8px)}to{opacity:1;transform:translateX(0)}}
@keyframes glare-fadeUp{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}
@keyframes glare-borderPulse{0%,100%{border-color:rgba(204,0,34,0.15)}50%{border-color:rgba(204,0,34,0.45)}}
@keyframes glare-cornerBlink{0%,49%,100%{opacity:0.4}50%,99%{opacity:1}}
@keyframes glare-typewriter{from{width:0}to{width:100%}}
@keyframes glare-shimmer{0%{background-position:200% center}100%{background-position:-200% center}}
@keyframes glare-spin{0%{transform:rotate(0deg)}100%{transform:rotate(360deg)}}
@keyframes glare-loadbar{0%{width:0%}100%{width:100%}}
.glare-enabled{animation:glare-borderPulse 2s ease infinite!important;border-color:rgba(0,180,80,0.5)!important;box-shadow:0 0 15px rgba(0,180,80,0.1)!important}
.glare-disabled{border-color:rgba(180,20,20,0.4)!important}
#GLARINGLY_GUI .scriptButton[style*="enabledButton"]{animation:glare-borderPulse 2.5s ease infinite}
.glare-toast{position:fixed;bottom:20px;right:20px;background:rgba(8,0,0,0.97);border:1px solid var(--crimson);border-radius:2px;padding:10px 18px;font-family:'Share Tech Mono',monospace;font-size:11px;color:#ff8888;letter-spacing:0.08em;z-index:99999;box-shadow:0 0 20px rgba(204,0,34,0.3);animation:glare-fadeUp .2s ease forwards;pointer-events:none}`
        }), u = l("div", {
            style: {
                width: "100%",
                height: "100%",
                position: "relative",
                outline: "1px solid rgba(204,0,34,0.35)",
                borderRadius: "4px",
                overflow: "hidden",
                boxShadow: "0 0 80px rgba(0,0,0,0.97), 0 0 40px rgba(204,0,34,0.08), 0 0 80px rgba(204,0,34,0.04), inset 0 0 0 1px rgba(204,0,34,0.06)"
            }
        }, l("div", {
            id: "background",
            style: {
                ...
                backgroundImage: "linear-gradient(rgba(204,0,34,0.04) 1px, transparent 1px), linear-gradient(90deg, rgba(204,0,34,0.04) 1px, transparent 1px)",
            ...
            id: "backgroundImage",
            style: {
                background: "radial-gradient(ellipse at 10% 50%, rgba(204,0,34,0.06) 0%, transparent 50%), radial-gradient(ellipse at 90% 10%, rgba(140,0,0,0.05) 0%, transparent 50%), radial-gradient(ellipse at 50% 90%, rgba(255,40,20,0.04) 0%, transparent 50%)",
                display: "block",
                height: "100%",
                position: "absolute",
                width: "100%",
            ...
            style: {
                ...
                borderBottom: "1px solid rgba(204,0,34,0.2)",
                borderRight: "1px solid rgba(204,0,34,0.15)",
            ...
            style: {
                ...
                width: "144px",
                background: "rgba(8,0,0,0.97)",
                borderLeft: "1px solid rgba(204,0,34,0.2)",
                borderTop: "1px solid rgba(204,0,34,0.2)",
            ...
            style: {
                ...
                width: "48px",
                fontFamily: "'Share Tech Mono', monospace",
                color: "rgba(204,0,34,0.8)",
                backgroundColor: "rgba(204,0,34,0.06)",
                border: "none",
                borderRight: "1px solid rgba(204,0,34,0.12)",
                fontSize: "1.4rem",
                cursor: "move",
                transition: "all .15s"
            },
            ...
            style: {
                ...
                border: "none",
                borderRight: "1px solid rgba(204,0,34,0.1)",
                fontSize: "1.8rem",
                fontWeight: "700",
                cursor: "pointer",
                transition: "all .15s"
            ...
            onclick: function() {
                    ...
                    return () => {
                        ...
                        u.style.height = e ? "100%" : "48px",
                            u.style.width = e ? "100%" : "144px",
                            _.style.top = parseInt(_.style.top) + (_.offsetHeight - 48) * (e ? -1 : 1) + "px",
                            _.style.left = parseInt(_.style.left) + (_.offsetWidth - 144) * (e ? -1 : 1) + "px",
            ...
            style: {
                ...
                borderRight: "1px solid rgba(204,0,34,0.18)",
                zIndex: "5",
                width: "210px",
                position: "absolute",
                top: "0",
            ...
            innerHTML: `<div style="padding:20px 14px 16px;border-bottom:1px solid rgba(204,0,34,0.2);background:linear-gradient(180deg,rgba(204,0,34,0.06),transparent);margin-bottom:6px;position:sticky;top:0;z-index:2;animation:glare-flicker 8s infinite"><div style="font-family:'Orbitron',sans-serif;font-size:15px;font-weight:900;letter-spacing:0.18em;color:#cc0022;text-shadow:0 0 15px rgba(204,0,34,0.9),0 0 30px rgba(204,0,34,0.5),0 0 60px rgba(204,0,34,0.2);line-height:1.2">GLARE<span style="color:#ff4422;font-size:11px;letter-spacing:0.3em;display:block;margin-top:2px;text-shadow:0 0 10px rgba(255,68,34,0.7)">CLIENT</span></div><div style="font-size:8px;letter-spacing:0.25em;color:rgba(204,0,34,0.4);font-family:'Share Tech Mono',monospace;margin-top:6px;text-transform:uppercase">by Glaringly // v2.0</div><div style="margin-top:8px;height:1px;background:linear-gradient(90deg,transparent,rgba(204,0,34,0.6),transparent)"></div></div>`
        }, l("a", {
            className: "bigButton",
            style: {
                cursor: "pointer",
            ...
            innerHTML: '<div style="background:rgba(204,0,34,0.05);border:1px solid rgba(204,0,34,0.2);border-radius:3px;padding:8px 12px;display:flex;align-items:center;justify-content:center;gap:6px;font-family:\'Share Tech Mono\',monospace;font-size:10px;font-weight:400;letter-spacing:0.15em;color:rgba(204,0,34,0.7);text-transform:uppercase;transition:all .2s">&#9654; GITHUB</div>'
        })), l("div", {
            ...
            style: {
                ...
                opacity: "0",
                textAlign: "center",
                boxShadow: "0 0 20px rgba(204,0,34,0.15)"
            },
            ...
            style: {
                ...
                color: "rgba(204,0,34,0.6)",
                background: "transparent"
            }
        })))))));
    for (let f of document.querySelectorAll("#GLARINGLY_GUI_OLD"))
        f.remove();

    function w(e, t, a, o) {
        let r = l("div", {
            className: "cheatButton",
            innerHTML: ("string" == typeof t ? `<img style="height: 24px; margin-right: 8px; opacity:0.8; filter:drop-shadow(0 0 4px rgba(204,0,34,0.5))" src="${t}">` : t || "") + e,
            onclick: () => {
                document.querySelectorAll("#GLARINGLY_GUI .cheatButton").forEach(b2 => b2.classList.remove("glare-active"));
                r.classList.add("glare-active");
                k(r.innerText, a, o);
            }
        });
    ...
    function insertElemBefore(name, imgSrc, a, o, elem) {
        let r = l("div", {
            className: "cheatButton",
            innerHTML: ("string" == typeof imgSrc ? `<img style="height: 24px; margin-right: 8px; opacity:0.8; filter:drop-shadow(0 0 4px rgba(204,0,34,0.5))" src="${imgSrc}">` : imgSrc || "") + name,
            onclick: () => {
                document.querySelectorAll("#GLARINGLY_GUI .cheatButton").forEach(b2 => b2.classList.remove("glare-active"));
                r.classList.add("glare-active");
                k(r.innerText, a, o);
    ...
    async function k(e, t, a) {
        b.innerHTML = "",
            v.firstChild.innerText = (e + (a ? "" : " // CHEATS")).toUpperCase(),
            b.append(v);
        for (let o = 0; o < t.length; o++) {
            ...
            if (!p) {
                ...
                if (u.dataset.description = i, u.onclick = (function({
                        ...
                        if (e == u || e.classList.contains("cheatName") || "Enter" == t && e.classList.contains("cheatInput")) {
                            // Ripple effect
                            const ripple = document.createElement("div");
                            const rect = u.getBoundingClientRect();
                            Object.assign(ripple.style, {
                                position: "absolute", borderRadius: "50%", background: "rgba(204,0,34,0.25)",
                                width: "8px", height: "8px", left: ((cx || rect.left + rect.width/2) - rect.left - 4) + "px",
                                top: ((cy || rect.top + rect.height/2) - rect.top - 4) + "px",
                                transform: "scale(0)", transition: "transform 0.4s ease, opacity 0.4s ease",
                                opacity: "1", pointerEvents: "none", zIndex: "10"
                            });
                            u.appendChild(ripple);
                            requestAnimationFrame(() => { ripple.style.transform = "scale(25)"; ripple.style.opacity = "0"; });
                            setTimeout(() => ripple.remove(), 400);

                            t = [...u.children].slice(1);
                            d.apply(this, t.map(e => "number" == e.type ? parseInt("0" + e.value) : "SELECT" == e.nodeName ? JSON.parse(e.value) : e.data || e.value));
                            if ("toggle" == n) {
                                u.style.background = this.enabled ? "var(--enabledButton)" : "var(--disabledButton)";
                                u.style.transform = "scale(0.96)";
                                setTimeout(() => u.style.transform = "", 120);
                                window.glareToast?.(this.enabled ? `✦ ${this.name} enabled` : `✧ ${this.name} disabled`);
                            } else {
                                window.glareToast?.(`▶ ${this.name} executed`);
                            }
                            C.alerts?.[0].addLog(`${"toggle" == n ? this.enabled ? "Enabled" : "Disabled" : "Ran"} <strong>${this.name}</strong>` + (s?.length ? ` with inputs: (${t.map(e => "SELECT" == e.nodeName ? e.selectedOptions[0].innerText : e.value).join(", ")})` : ""), "toggle" == n ? this.enabled ? "var(--enabledButton)" : "var(--disabledButton)" : null);
                        }
                    }).bind(t[o]), s?.length)
            ...
            const el = t[o].element;
            el.style.opacity = "0";
            el.style.transform = "translateY(8px)";
            el.style.transition = `opacity 0.2s ease ${o * 40}ms, transform 0.2s ease ${o * 40}ms`;
            b.appendChild(el);
    ...
    // === GLARE CLIENT ENTRY ANIMATION ===
    _.style.opacity = "0";
    _.style.transform = `scale(${c.data.scale || 1}) translateY(-12px)`;
    _.style.transition = "opacity 0.35s cubic-bezier(.4,0,.2,1), transform 0.35s cubic-bezier(.4,0,.2,1)";
    requestAnimationFrame(() => requestAnimationFrame(() => {
        _.style.opacity = "1";
        _.style.transform = `scale(${c.data.scale || 1}) translateY(0)`;
    }));

    // === CORNER DECORATIONS ===
    const corners = [
        { top: "0", left: "0", borderTop: "2px solid var(--crimson)", borderLeft: "2px solid var(--crimson)", borderRadius: "4px 0 0 0" },
        { top: "0", right: "0", borderTop: "2px solid var(--crimson)", borderRight: "2px solid var(--crimson)", borderRadius: "0 4px 0 0" },
        { bottom: "0", left: "0", borderBottom: "2px solid var(--crimson)", borderLeft: "2px solid var(--crimson)", borderRadius: "0 0 0 4px" },
        { bottom: "0", right: "0", borderBottom: "2px solid var(--crimson)", borderRight: "2px solid var(--crimson)", borderRadius: "0 0 4px 0" }
    ];
    corners.forEach(s => {
        const c2 = document.createElement("div");
        Object.assign(c2.style, { position: "absolute", width: "10px", height: "10px", zIndex: "10", pointerEvents: "none", opacity: "0.7", animation: "glare-cornerBlink 3s ease infinite", ...s });
        u.appendChild(c2);
    });
    ...
    Object.assign(scanline.style, {
        ...
        background: "linear-gradient(transparent, rgba(204,0,34,0.06), transparent)",
        pointerEvents: "none", zIndex: "9", animation: "glare-scan 6s linear infinite", opacity: "0.6"
    });
    u.appendChild(scanline);

    // === TOAST NOTIFICATION HELPER ===
    window.glareToast = function(msg, dur = 2500) {
        const t2 = document.createElement("div");
        t2.className = "glare-toast";
        t2.textContent = "// " + msg;
        document.body.appendChild(t2);
        setTimeout(() => { t2.style.transition = "opacity 0.3s"; t2.style.opacity = "0"; setTimeout(() => t2.remove(), 300); }, dur);
    };
    ...
    let C = {
        ...
        settings: [{
            ...
            run: function(e) {
                ...
                for (let a in e = {
                        backgroundColor: "linear-gradient(135deg,#0d0d1a 0%,#0a0a1f 60%,#0d0d24 100%)",
                        infoColor: "#0d0d1a",
                        cheatList: "#0a0a1f",
                        defaultButton: "#1a1a3a",
                        disabledButton: "#8b0025",
                        enabledButton: "#00602a",
                        textColor: "#e0e0ff",
                        inputColor: "#0d1230",
                        contentBackground: "rgba(10,10,30,0.85)",
                        ...JSON.parse(e)
                    }, c.setItem("theme", e), e)
            ...
            inputs: [{
                ...
                options: [{
                    ...
                    name: "glaringly Neon",
                    value: {
                        backgroundColor: "radial-gradient(ellipse at 20% 50%,#12003f,#000918)",
                        infoColor: "rgba(204,0,34,0.12)",
                        cheatList: "rgba(204,0,34,0.07)",
                        defaultButton: "rgba(124,63,255,0.35)",
                        disabledButton: "rgba(255,45,120,0.4)",
                        enabledButton: "rgba(0,255,136,0.35)",
                        textColor: "#e0e0ff",
                        inputColor: "rgba(204,0,34,0.08)",
                        contentBackground: "rgba(0,0,20,0.9)"
                    }
                }, {
                    ...
                    value: {
                        ...
                        disabledButton: "#4a0808",
                        enabledButton: "#0a3a0a",
                        textColor: "white",
                        inputColor: "#3f3f3f",
   ...
   w("Alerts", "https://res.cloudinary.com/dhiws7ac5/image/upload/v1743434255/alerts_dsucpi.png", C.alerts, !0),
    ...
    [...$.children].forEach(btn => {
        btn.addEventListener("mouseenter", () => { btn.style.filter = "brightness(150%)"; btn.style.background = "rgba(204,0,34,0.12)"; });
        btn.addEventListener("mouseleave", () => { btn.style.filter = ""; btn.style.background = btn === [...$.children][0] ? "rgba(204,0,34,0.06)" : "transparent"; });
    });

    // Auto-activate first nav item
    setTimeout(() => {
        const first = h.querySelector(".cheatButton");
        if (first) { first.classList.add("glare-active"); first.click(); }
    }, 50);
    let x = setInterval(() => {
    ...
    _.addEventListener("mousemove", e => {
        ...
        window.fetch.call = function() {
            if (!arguments[1]?.includes("s.blooket.com/rc"))
                return wfcall.apply(this, arguments);
            C.alerts?.[0].addLog("⚡ Cheat Report Blocked by glaringly!")
        }
})();
